...
public class FileServiceImplV2 extends FileServiceImplV1{
	...
	public int delete(String imageFileName) {
		 ...
		 * 파일의 null값 등을 검사하여 실행을 취소하는 방법이 
		 * 더 편리한 코드가 될 것이다
		 * 	
		 */
		...
		File delFile = new File(this.fileUpPath, imageFileName);
		if(delFile.exists()) {
			...
			if(ok) {
				 ...
				 * 실패하는 메세지를 return 할때 null을 return 하면된다
				 * 
				 * primitive 숫자형으로 return type을 설정한 경우
				 * null 값을 return 할 수 없다
				 * 
				 * 이럴때 성공한 메세지는 양의 정수값을 return 하고 
				 * 실패한 메세지는 음의 정수값을 return 한다.
				 * 
				 * 호출한 곳에서 성공실패를 검사할때
				 * if(ret > 0) 성공
