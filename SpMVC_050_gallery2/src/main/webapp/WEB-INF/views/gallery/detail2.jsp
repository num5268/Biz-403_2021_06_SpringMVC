...
<script>
	...
	let update_delete = document.querySelector("button.gallery.delete")
	...
	update_button.addEventListener("click",()=>{
		...
		이 코드는 JS문법어류가 발생할 것이다
		
		왜 ??
				
		현재 작성한 코드는 JSP 코드이다
		이 코드는 Response가 될때 HTML코드로 변환이 되고
		Script부분도 변환이 되는데
		
		변환되는 과정에서 S{GALLERY.g_seq}는 담겨있는 문자열인
		S0001만 단독으로 나타난다
		
		아래 코드는 "/root-context/gallery/update?g_seq"+ S0001처럼 
		변환이 된다
		
		결국 JS코드가 실행될때 + S0001 처럼 변환되어 변수를 찾게된다
		...
		*/
		location.href 
		= "${rootPath}/gallery/update?q_seq=${GALLERY.g_seq}"
	...
	한가의 선언된 변수에 코드 중간에 다른 값이 저장되어 
	논리적인 오류를 일으 킬수 있다 
	그러한 문제를 방지하기 위하여 const
	*/
		...
		gallery_files.addEventListener("click",(e)=>{
			...
			if(tag.tagName === "DIV" && tag.className.includes("gallery_file")){
				...
				if(confirm(seq + "이미지 삭제 할까요?")){
					...
					fetch("${rootPath}/gallery/file/delete/"+ 'A001') //+ seq)
