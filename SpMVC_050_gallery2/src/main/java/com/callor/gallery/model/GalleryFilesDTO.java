...
public class GalleryFilesDTO {
	...
	/*
	 * tbl_gallery와 tbl_files table을 join하여 select한 데이터는
	 * 실제로는 1:N 관계로 생성된 결과이다
	 * 
	 * 하지만 실제 보여지는 List는 마치 tbl_gallery가 여러개 있는 것처럼 보인다
	 * 
	 * 예) gallery 1개 데이토에 file이 3개 첨부된 경우
	 * 보여지는 List는 다음과 같다
	 * ===========================
	 * 1번 gallery		1번 파일
	 * 1번 gallery		2번 파일
	 * 1번 gallery		3번 파일
	 * ===========================
	 * 실제로는 gallery는 1개 이지만 추출된 데이터는 전체가 List이므로
	 * 전체 view를 담을 DTO를 선언하고 List<DTO>형 변수에 데이터를 담았다
	 * 
	 * 실제 데이터와 상관없이 g_seq 등등 gallery정보를 담을 변수들은
