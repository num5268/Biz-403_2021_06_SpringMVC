...
public class GalleryServiceImplV2 extends GalleryServiceImplV1{
	...
	/*
	 * @RequireArgConstructor 문제로 상속받을 때는
	 * 상속받은 클래스에서 강제로 생서자를 만들어야 함
	 * 
	...
	public int delete(Long g_seq) {
		...
		// 본문 첨부파일 삭제
		String imageFileName = gaDTO.getG_image();
		int ret= fService.delete(imageFileName);
		if(ret > 0) {
			// 본문 데이터 삭제
			gaDao.delete(g_seq);
		} else {
			log.debug("파일삭제 실패로 데이터 삭제 하지 않음");
		}
