...
public class ApiController {
	...
	/*
	 * Ajax를 통해서 전송되어온 데이터 받기
	 * Ajax POST로 전송된 데이터는  Request의 body(payLoad)에 담겨서
	 * 전달되어 온다
	 * 일번적인 JSP(form, link)에서 보낸 데이터는
	 * 매개변수를 통하여 직접 전달받을수 있는데
	 * Ajax를 통하여 받은 데이터는 그렇지 못한다
	 * 무조건 1개의 변수에 모든 데이터가 JSON 문자열 형태로 받아진다
	 * 
	 * JSON 문자열을 JSON, 객체 형식으로 변환(parsing)해야 하는
	 * 문제에 봉착한다.
	 * 
	 * Spring에서 Ajax로 전송된 데이토는 무조건 Map으로 받는다
